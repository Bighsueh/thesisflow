...
export const cohortService = {
  ...
  createCohort: async (payload: { name: string; code?: string; project_id?: string | null }): Promise<Cohort> => {
    return api.post('/api/cohorts', payload);
  },
  updateCohort: async (cohortId: string, payload: { name?: string; code?: string | null; project_id?: string | null }): Promise<Cohort> => {
  ...
  updateCohortMember: async (cohortId: string, userId: string, payload: { status?: string; progress?: number }): Promise<void> => {
