...
- **適用情境**：凡涉及 `docker-compose.yml`、環境變數、資料庫 schema/migration、FastAPI 後端模組（`backend/auth.py`, `backend/models.py`, `backend/schemas.py`, `backend/services.py`）、核心 React 狀態或共用元件（`frontend/store.ts`, `frontend/authStore.ts`, `frontend/components/`, `frontend/pages/`）的程式碼變更。
- **必須執行完整流程的時機**：
  - 準備送出 PR 或合併到 `main` 前。
...
1. **系統級設定變更**：`docker-compose.yml`、`frontend/Dockerfile`、`backend/Dockerfile`、`env` 檔、啟動腳本。
2. **資料庫相關變更**：`backend/models.py`, `migration_*.sql`, `backend/db.py`, schema 說明文件。
3. **後端 API / 商業邏輯變更**：`backend/main.py`, `backend/services.py`, `backend/schemas.py`, 任何 FastAPI router。
4. **前端核心狀態或共用元件變更**：`frontend/store.ts`, `frontend/authStore.ts`, `frontend/pages/*`, `frontend/components/*`, `frontend/components/widgets/*`.
5. **Auth / 權限 / 角色流程變更**：`backend/auth.py`, token 邏輯、角色判斷、前端 Route 守門、`frontend/authStore.ts`.

## 3. 各類變更檢查清單

### 3.1 系統級設定變更

- **應檢查組件**：`docker-compose.yml`, `.env*`, `backend/Dockerfile`, `frontend/Dockerfile`, `frontend/vite.config.ts`, `check_env.sh`.
- **檢查項目**：
...
- **應檢查組件**：`backend/models.py`, `backend/db.py`, SQL migration, `backend/schemas.py`, `backend/services.py`, 受影響的前端表單/列表。
- **檢查項目**：
  ...
  - [ ] `backend/db.py` 連線/Session 管理是否因 schema 變更需要調整（如加大 pool）。
- **未檢查可能後果**：API 500、資料遺失或無法序列化、舊資料造成 migration 失敗、前端顯示空白或崩潰。

### 3.3 後端 API / 商業邏輯變更

- **應檢查組件**：`backend/main.py` router、`backend/services.py`, `backend/schemas.py`, 單元測試/整合測試、前端呼叫這些 API 的頁面與 `frontend/store.ts`.
- **檢查項目**：
  - [ ] 新增/調整的 endpoint 有相對應 schema、驗證與對應 service 層。
  - [ ] 負責操作的 `services` 方法在交易/錯誤處理/邊界條件上都有更新。
  - [ ] 影響的前端頁面（如 `frontend/pages/TeacherCohort`, `frontend/components/EvidenceCreateDialog`）是否更新 API URL、HTTP method、payload。
  - [ ] 回應格式變化是否在前端解析器、`frontend/store.ts` 中同步更新，避免 undefined 欄位。
...
- **應檢查組件**：`frontend/store.ts`, `frontend/authStore.ts`, `frontend/pages/*`, `frontend/components/*`, `frontend/hooks/useAutoSave.ts`, `frontend/components/widgets/*`.
- **檢查項目**：
  - [ ] Global store state shape 是否改變，並同步更新使用該 state 的所有 selector/hook。
  - [ ] 重要元件（例如 `frontend/components/ChatMainPanel`, `frontend/components/EvidenceListPanel`）在 props、context、async flow 變更後是否有 loading/error fallback。
  - [ ] 路由（`frontend/App.tsx`, `frontend/pages/`）與權限守門邏輯是否對應新狀態。
  - [ ] 若牽涉檔案上傳、PDF顯示，需檢查跨瀏覽器與大型檔案的行為。
  - [ ] Storybook/單元測試/型別定義（`frontend/types.ts`）是否更新。
- **未檢查可能後果**：頁面崩潰、狀態不同步造成資料遺失、權限頁面顯示錯誤內容、TypeScript 編譯錯誤。

### 3.5 Auth / 權限 / 角色流程變更

- **應檢查組件**：`backend/auth.py`, `backend/services.py` 的角色判斷、`backend/schemas.py` 的 token payload、前端 `frontend/authStore.ts`, `frontend/pages/LoginPage`, `frontend/pages/TeacherHome`, `frontend/pages/StudentHome`, Route Guard。
- **檢查項目**：
...
- 只修改 `frontend/store.ts` 而未更新使用該 state 的 selector，導致 runtime `undefined`.
