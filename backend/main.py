...
# Workflow State
@app.get("/api/workflow-state/{project_id}", response_model=schemas.WorkflowStateOut | None)
def get_workflow_state(project_id: str, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
...
@app.post("/api/workflow-state", response_model=schemas.WorkflowStateOut)
def create_or_update_workflow_state(payload: schemas.WorkflowStateCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    existing_state = db.query(models.WorkflowState).filter(
        models.WorkflowState.project_id == payload.project_id,
        models.WorkflowState.user_id == current_user.id
    ).first()
...
@app.patch("/api/workflow-state/{project_id}", response_model=schemas.WorkflowStateOut)
def update_workflow_state(project_id: str, payload: schemas.WorkflowStateUpdate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    """部分更新 workflow 狀態"""
    state = db.query(models.WorkflowState).filter(
        models.WorkflowState.project_id == project_id,
        models.WorkflowState.user_id == current_user.id
