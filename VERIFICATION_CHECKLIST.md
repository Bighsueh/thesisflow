...
3. **驗證過濾邏輯**
   ...
   - **不應該**看到「學習任務工作區導覽」（student-interface）
...
## 測試 3：StudentInterface Navbar 與學習任務工作區導覽
...
| 狀態 | 預期行為 | 驗證 |
|------|---------|------|
| **在 `/student/learning-task`** | HelpCenter 中**只顯示**「學習任務工作區導覽」 | ☐ |
| **不顯示其他導覽** | 不應看到「儀表板導覽」、「文獻導覽」、「學習任務導覽」、「群組導覽」 | ☐ |
