...
3. **啟動跨頁面導覽**
   - 在 HelpCenter 中點擊「**專案工作區導覽**」（student-interface）
   - 觀察以下行為：
...
| 項目 | 預期行為 | 驗證 |
|------|---------|------|
| **Modal 關閉** | HelpCenter modal 應立即關閉 | ☐ |
| **頁面導航** | 頁面應自動導航到 `/student/project` | ☐ |
| **導覽自動啟動** | 約 1.1 秒後，導覽應自動啟動（高亮元素） | ☐ |
| **Console 日誌** | 應看到 `[HelpCenter] Navigating from /dashboard to /student/project` | ☐ |
| **無警告** | **不應**出現 5 次「Target not found」重複警告 | ☐ |
| **降級機制** | 如果目標元素未找到，應顯示**中央卡片**而非崩潰 | ☐ |
...
3. **啟動同頁面導覽**
   - 點擊「**儀表板導覽**」（dashboard-intro）
   - 應該**立即**看到導覽動畫開始（無頁面重新加載）

### 預期結果

| 項目 | 預期行為 | 驗證 |
|------|---------|------|
| **無導航延遲** | 導覽應在 300ms 內啟動 | ☐ |
| **頁面保持** | 頁面路徑保持 `/dashboard` | ☐ |
| **Console 日誌** | 應看到 `[HelpCenter] Starting tour on current page` | ☐ |

---

## 測試 3：StudentInterface Navbar 與 HelpCenter 路由過濾

### 測試步驟

1. **訪問 StudentInterface**
   - 在瀏覽器中打開 http://localhost:3000/student/project
   - 或使用跨頁面導覽自動導航（測試 1）

2. **驗證 Navbar 組件**
   - 頁面頂部應顯示完整的 navigation bar

3. **驗證 HelpCenter 路由過濾**
   - 在 `/student/project` 頁面點擊 HelpButton
...
| 頁面 | 路由 | 預期導覽 | 驗證 |
|------|------|---------|------|
| **儀表板** | `/dashboard` | 只顯示「儀表板導覽」 | ☐ |
| **文獻庫** | `/literature` | 只顯示「文獻導覽」 | ☐ |
| **專案列表** | `/projects` | 只顯示「專案導覽」 | ☐ |
| **群組管理** | `/groups` | 只顯示「群組導覽」 | ☐ |
